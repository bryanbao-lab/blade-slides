...
# BLADE Slides V2.1.1｜高保真可编辑 PPT 分层还原
...
在已验收的 BLADE deck 上做局部修正，不能重做已经成功的页面。若只修改指定对象，记录其语义名称与允许修改的属性，同页其他对象也受保护（见方法论的局部优化说明）。记录：
...
2. **规则几何原生化：** 无复杂材质的普通直线、边框、卡片、坐标轴和箭头优先用安全原生形状；不能仅因轮廓是矩形，就将玻璃、金属或光效壳体判为普通几何。
...
如果原生形状无法还原切角、金边、玻璃和发光质感，可以使用“高保真透明壳体 + 原生文字/icon”的混合方案；壳体必须完整、干净、四角透明且有安全边距。玻璃叠层按方法论的面板规则检查实际合成透明度，不能只检查 PNG 是否有 alpha。
